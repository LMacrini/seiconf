  ...
  config.perSystem =
    ...
    let
      nixAttr =
        ...
        in
        if builtins.isAttrs value && builtins.length (builtins.attrNames value) == 1 then
          let
      ...
      nixCode =
        ...
        if lib.isStringLike expr then
          ...
          map (expr: nixCode expr) expr |> (values: "[${builtins.concatStringsSep " " values}]")
